...
"""prepare_gmq_input_file.py: 
            prepares GMQ output training file 
            the code here is an exact copy of prepare_gmq_test_file.py, I just seperated them to generate training and testing data simultaneously 
...
if __name__ == '__main__':
    ...
    input_feature_file = out_dir+"data_file/train_May4"
    os.chdir(out_dir)
    ...
    #output_file = open("train1_"+str(ca_cutoff)+"_"+str(neighbor_cutoff)+"_"+str(weight_list[0])+                   "_"+str(weight_list[1])+"_"+str(weight_list[2])+"_"+str(weight_list[3]),"w")    
    output_file = open('gmq_train_may4','w')
    for i in range(len(target_model_list)):


        residue_indices = write_residue_labels_to_output_file(output_file,target_model_list[i],                                               total_residue_count[i],input_feature_file,                                               ca_cutoff)

        write_features_to_output_file(output_file,target_model_list[i],total_residue_count[i],                         input_feature_file,neighbor_cutoff,num_neighbor,residue_indices,                         weight_list)
